...
export class OrderedBulkInserts implements BenchmarkSuite {
  name = 'Ordered Bulk Inserts';
  group = 'write';

  async setup(ctx: BenchmarkContext) {
    await ctx.db.collection<BenchDoc>('bench_write_ordered').drop().catch(() => {});
  }

  async run(ctx: BenchmarkContext): Promise<BenchmarkResult> {
    const { bulkBatchSize, bulkBatches, warmupIterations } = ctx.config;
    const col = ctx.db.collection<BenchDoc>('bench_write_ordered');
    let counter = 0;
    ...
    async function makeBatch() {
      ...
      for (let i = 0; i < bulkBatchSize; i++) {
        ...
        docs.push({
          ...
          email: `bulk_o${counter}@bench.test`,
          score: counter % 10000,
    ...
    for (let i = 0; i < Math.min(warmupIterations, 2); i++) {
      await col.insertMany(await makeBatch(), { ordered: true });
    }
    ...
    for (let i = 0; i < bulkBatches; i++) {
      ...
      await recorder.measure(() => col.insertMany(docs, { ordered: true }));
